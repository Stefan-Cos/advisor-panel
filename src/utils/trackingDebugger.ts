...
export const debugTrackingScripts = () => {
  ...
  fbScripts.forEach((script, index) => {
    console.log(`FB Script ${index + 1}:`, (script as HTMLScriptElement).src);
  });
  ...
  fbNoscripts.forEach((img, index) => {
    console.log(`FB Pixel ${index + 1}:`, (img as HTMLImageElement).src);
  });
  ...
  trackingImages.forEach((img, index) => {
    console.log(`Tracking Image ${index + 1}:`, (img as HTMLImageElement).src);
  });
